...
from pyspark.sql.functions import explode, arrays_zip, row_number, col, sum
...
df_4 = dff.selectExpr("itemId", "cast(availableQuantity as double) availableQuantity")

df_percent = df_4.withColumn('stockPercentage', col('availableQuantity') / sum('availableQuantity').over(Window.partitionBy()) * 100)
df_percent.withColumn('stockPercentage', round(col('stockPercentage'), 2)) \
